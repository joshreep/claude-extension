...
- **Guard/conditional removal — secondary-effect check**: if the diff removed or weakened a conditional (`*ngIf`, `if`, `??.`, `&&`, early return, optional chaining), verify the component's or service's initialization defaults are correct without it. A removed guard can expose stale default values (e.g., numeric defaults like `10` now visible to users) or uninitialized properties that were previously masked. Flag as **Major** if any default is now user-visible in an incorrect state. Note: the reviewer should have caught this — if it appears here, flag it as a reviewer miss too.
